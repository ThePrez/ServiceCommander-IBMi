...
```
SBMJOB CMD(CALL PGM(QP2SHELL2) PARM('/QOpenSys/usr/bin/sh' '-c' 'yes "sleep 40 && /QOpenSys/pkgs/bin/sc start navigator >/dev/null 2>&1" | sh')) JOB(NAVMON) JOBD(*USRPRF) JOBQ(QUSRNOMAX)
```
