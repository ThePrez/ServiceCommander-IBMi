...
public class YamlServiceDef extends ServiceDefinition {
    ...
    public String getBatchJobName() {
        ...
        if (StringUtils.isNonEmpty(m_batchJobName)) {
            return m_batchJobName;
        }
        // Else, let's just go with an SC_ autogenerated thing, because, why not?
        return ("SC_" + getName() + "          ").replace("-", "").substring(0, 10).trim();
    }
